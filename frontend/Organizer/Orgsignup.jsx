...
const navigate = useNavigate();
 ...
 const  handlesubmit= async(e) => { 
    ...
    try { 
        ...
        const { data } = await axios.post(`${backendurl}/organizer/organizer-signup`, details);

         if(data.success===true){
             toast.success("Organizer SignedIn ") 
        ...
        seterror(error?.response?.data?.message || " Organizer Signup failed")
    }
 }
  ...
  return <>   
   ...
   <input type="password" placeholder="Enter Password "   value={password} 
        ...
        className="w-full px-4 py-2 bg-slate-700 text-white placeholder-slate-300 border border-slate-600 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-400 mb-4" 
          ...
          <button type="submit" className="w-full bg-blue-600 text-white py-2 rounded-md hover:bg-slate-700 transition"> Signup Organizer </button> 
